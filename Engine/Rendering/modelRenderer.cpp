...
void ModelRenderer::Load(const char* FileName, bool dynamic)
{
	if (m_ModelPool.count(FileName) > 0)
	{
	...
	m_Model = new MODEL;
	LoadModel(FileName, m_Model, dynamic);

	m_ModelPool[FileName] = m_Model;

}

void ModelRenderer::LoadModel(const char* FileName, MODEL* Model, bool dynamic)
{
